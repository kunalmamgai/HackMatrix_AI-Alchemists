...
export default function DrawerEstimator({ darkMode }) {
  ...
  return (
    <section id="drawer-estimator" className={`py-20 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-10 items-start">
              ...
              {devices.map((device, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`relative text-left rounded-2xl p-5 border-2 transition-all ${
                      isSelected
                        ...
                        : darkMode
                          ? 'border-gray-700 bg-gray-800 hover:border-eco-500/50'
                          : 'border-gray-200 bg-white hover:border-eco-500/50'
                    }`}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                  ...
                  >
                    ...
                    />
                    <h3 className={`font-bold mb-0.5 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {device.name}
                    </h3>
          ...
          {/* Live total */}
          <motion.div
            className={`lg:sticky lg:top-24 rounded-3xl p-8 shadow-xl border-2 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            ...
            {selectedDevices.length === 0 ? (
              <div className={`text-center py-8 px-4 rounded-2xl border-2 border-dashed ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
              ...
              <>
                <div className="space-y-5">
                  <div>
                    ...
                    <p className={`text-3xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      ...
                      <span className="text-lg font-semibold text-gray-500"> / {devices.length}</span>
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className={`text-3xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      ...
                      <span className="text-base font-semibold text-gray-500"> CO₂e</span>
                    </p>
                  </div>

                  <div className={`rounded-2xl p-4 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                    <p className={`text-sm font-semibold mb-2 flex items-center gap-1.5 ${darkMode ? 'text-eco-300' : 'text-eco-700'}`}>
                ...
                <button
                  ...
                  className="mt-6 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border-2 text-sm font-semibold transition-colors border-gray-600 text-gray-400 hover:border-red-500 hover:text-red-400"
                >
                  <RotateCcw size={16} />
                  Reset
                </button>
